...
test("custom JSON adapter bounds interpolation, normalizes relative URLs, and enforces same-run provenance", async () => {
  ...
  const search = await tools.searchJobs.execute("search", { query: "backend engineer&bad", location: "Jakarta", limit: 5 }, undefined);
  ...
  const detailResult = await tools.fetchJobDetails.execute("detail", { resultId: "42" }, undefined);
  ...
  await assert.rejects(tools.fetchJobDetails.execute("fabricated", { resultId: "not-returned" }, undefined), /was not returned by searchJobs/);
  detail = { ...detail, id: "fabricated" };
  await assert.rejects(tools.fetchJobDetails.execute("bad-id", { resultId: "42" }, undefined), /provenance mismatch/);
  detail = { ...detail, id: "42", url: "/jobs/other" };
  await assert.rejects(tools.fetchJobDetails.execute("bad-url", { resultId: "42" }, undefined), /provenance mismatch/);
});
...
test("custom HTML adapter extracts bounded selectors and relative links", async () => {
  ...
  const search = await tools.searchJobs.execute("search", { query: "platform", location: "", limit: 1 }, undefined);
  const searchBlock = search.content[0];
  ...
  const detail = await tools.fetchJobDetails.execute("detail", { resultId: "html-1" }, undefined);
