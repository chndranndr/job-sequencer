...
export function buildWriterPrompt(input: {
  ...
  const sections = [
    trustedSection("INSTRUCTIONS", [
      ...
      "The summary must be specific to this posting: lead from strategy positioning and the primarySellingPoints that answer this role's needs, in that order of importance. Never restate or lightly paraphrase the identity:summary evidence item; it is the same static text for every job, so reusing it produces an untailored CV. The identity:summary EvidenceRef may still be cited to satisfy grounding; only the wording must be job-specific. Sell with grounded specifics from cited evidence, not superlatives or self-praise; do not claim seniority, impact, or scope beyond what the cited evidenceRefs state.",
