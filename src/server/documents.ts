...
export type CommandRunner = (executable:string,args:string[],timeoutMs?:number,cwd?:string)=>Promise<CommandResult>;
export type DocumentVerification = { success:boolean; cvPages:number; coverLetterPages:number; cvTextPresent:boolean; coverLetterTextPresent:boolean; emailPresent:boolean; phonePresent:boolean; checkedAt:string };
...
export async function runCommand(executable: string, args: string[], timeoutMs = 10_000, cwd?: string): Promise<CommandResult> {
...
async function checked(runner:CommandRunner,exe:string,args:string[],cwd:string){ const result=await runner(exe,args,30_000,cwd); if(result.code!==0)throw new Error(`${exe} failed.`); return result; }
export async function compileAndVerify(options:{currentDir:string;cvPages:number;coverLetterPages:number;email:string;phone:string;runner?:CommandRunner;now?:string}):Promise<DocumentVerification>{
  ...
  await checked(runner,"lualatex",["-interaction=nonstopmode","-halt-on-error","cv.tex"],options.currentDir);
  await checked(runner,"xelatex",["-interaction=nonstopmode","-halt-on-error","cover-letter.tex"],options.currentDir);
  ...
  const cvInfo=await checked(runner,"pdfinfo",["cv.pdf"],options.currentDir), letterInfo=await checked(runner,"pdfinfo",["cover-letter.pdf"],options.currentDir);
  const cvText=(await checked(runner,"pdftotext",["cv.pdf","-"],options.currentDir)).stdout.trim();
  const letterText=(await checked(runner,"pdftotext",["cover-letter.pdf","-"],options.currentDir)).stdout.trim();
  ...
  return {success:true,cvPages:cvCount,coverLetterPages:letterCount,cvTextPresent:true,coverLetterTextPresent:true,emailPresent:true,phonePresent:true,checkedAt:options.now??new Date().toISOString()};
}
